...
function GetAXMaterails(const XMLPrimaryKey: Widestring):Boolean;//获取客户信息
      ...
      try
        ...
        begin
          if LoadAXCustomer(FListA.Values['Z_OrgAccountNum'],FListA.Values['Z_CompanyId']) then
            WriteLog(FListA.Values['Z_OrgAccountNum']+'客户信息下载成功')
          else
            WriteLog(FListA.Values['Z_OrgAccountNum']+'客户信息下载失败');
          if LoadAXSalesContract(FListA.Values['Z_CID'],nDataAreaID) then
            WriteLog(FListA.Values['Z_CID']+'销售合同下载成功')
          else
            WriteLog(FListA.Values['Z_CID']+'销售合同下载失败');
        end;}
      except
        on e:Exception do
        begin
